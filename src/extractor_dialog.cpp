...
const char *k_system_prompt =
  ...
  "   **The extension is not at the end of the url.** A real manifest address "
  "carries a query string — `…/master.m3u8?token=…&e=…` — so a test anchored "
  "with `$` matches nothing on the sites this exists for. Accept the "
  "extension followed by `?`, `&`, `#` or the end: `/\\.(m3u8|mpd)(\\?|&|#|$)/` "
  "or `indexOf('.m3u8') !== -1`. Measured: of the scripts kept from a run of "
  "this suite, the only one that anchored its fallback was the only one "
  "rejected, and its sibling clause could not match the real url either.\n"
