...
**The settings UI exists.** §11.3's player radio group is built as described — supported players listed, installed ones selectable, missing ones greyed with the reason, default resolved from what is present — including the **Custom…** command template (`%U` for the URL, appended when omitted; whitespace-split, not a shell). The §11.4 BitTorrent knobs are reachable too: connection caps, seed ratio, sequential-by-default and `listen_interfaces`, each carrying its reasoning next to the control. Persisted via QSettings and applied at startup rather than by the dialog, so they hold on a run where nobody opens it.

**Watch-while-downloading works for torrents.** The §11.3 growing-file path now takes a torrent as its source: `prioritize_streaming` sets sequential order *and* piece deadlines on the opening pieces, and the local proxy serves the file behind a readable-prefix callback. That callback is not optional polish — libtorrent allocates files sparse and full-size, so trusting `QFile::size()` serves the player zeros out of the holes, which presents as a corrupt stream rather than a slow one. Expressed on the seam as `streamable` / `prioritize_streaming` / `contiguous_bytes` so it is a property of sources rather than of BitTorrent; HTTP downloads qualify for the same reason.

**In-page magnet links are handled.** `web_view_factory::set_external_url_handler` is the seam for "a URL the engine will not render as a page"; the shell's rule is deliberately general — anything not renderable that some download source will take becomes a download — so it names no transport. It sits on the factory rather than a view because on desktop the mechanism is genuinely profile-wide (see §11.4 on why the per-navigation callback cannot work), while Android's `shouldOverrideUrlLoading` satisfies the same interface per view.
...
**Trigger surfaces.** Three. A `magnet:` URI is a navigation the shell takes over before the engine tries and fails to load it — **but not by intercepting the navigation**, which was the obvious guess and is wrong. Chromium classifies unregistered schemes as *external protocols* and disposes of them before any per-navigation callback runs: measured against Qt 6.8, `QWebEnginePage::navigationRequested` is never invoked for `magnet:` while it fires normally for `http`, and the click ends up at the desktop's mail/torrent handler instead. The mechanism that does work is `QWebEngineUrlScheme::registerScheme()` before the engine initialises, plus a `QWebEngineUrlSchemeHandler` on the profile — which catches both a clicked link and a scripted `window.location`. Registration must therefore happen in `main()`, and only for schemes a source can actually take: a registered scheme with no handler behind it swallows the click silently, which is worse than the error page. A `.torrent` link is an ordinary download the manager already receives. A response typed `application/x-bittorrent` is only visible with the local proxy tier (§10) — request-only interception sees the URL, not the Content-Type, so extension-based detection is the fallback exactly as it is for media.
...
1. **Embed a library.** ⚠️ **Two different libraries are both called "libtorrent" — check which one any advice refers to.**
   ...
   **Naming hazard worth writing down:** both packages install a pkg-config file named `libtorrent`. `pkg-config --cflags libtorrent` resolves to **rakshasa's**; rasterbar is `libtorrent-rasterbar`. Always ask for the qualified name. CMake's `find_package(LibtorrentRasterbar)` used to be the way out of this, and with CMake gone the discipline is the whole safeguard: `hydra.pro` reaches for the qualified name, and `torrent_download_source.cpp` carries an `@pkg_optional libtorrent-rasterbar` beside the include so fmake asks for the same one.
...
It is source and tooling, not a build dependency: no build file refers to it. At runtime, prefer a `yt-dlp` on `PATH` (the §11.3 player-probe pattern) and fall back to the vendored copy under the system Python; with neither, the feature degrades to the AI path alone.
...
`network_fetcher` is that bridge, and it marshals with `Qt::BlockingQueuedConnection` onto a thread of its own which spins a nested event loop for the reply. Nested is safe *there* and nowhere else: on the calling thread the same loop would deliver paint events and user input into the middle of a running script. Two consequences follow and are enforced rather than documented. Blocking from a thread to itself deadlocks, so `fetch` refuses that case and returns a failure — a deadlock inside a review dialog is indistinguishable from a hang. And **a blocked thread serves nothing** — which the test suite found the hard way, by blocking the very thread its own fake origin was listening on and watching every request "time out". The script therefore runs on a thread of its own too: `extractor_dialog` judges a proposal off the UI thread and posts the verdict back through a `QPointer`, so a dialog closed mid-run finds a null guard and stops rather than writing into freed widgets. The pure tier takes the same path, since one path is easier to reason about than two and a runaway script is bounded by the watchdog either way. The property is asserted as an ordering rather than a feeling: a timer on the UI thread must fire *before* the verdict arrives, and it does — 120 ms against a script that blocked until 410 ms.

The one contract this leaves the caller: a `helper_host` must outlive the run it was handed to, since the worker holds it by pointer. The evidence for that is in `project.md`: four prompt iterations were needed to get a *synchronous* two-argument function reliably.
...
**Status: done**, unexercised against a live KeePassXC. `keepass_protocol` holds the wire format and nonce discipline as pure functions, `crypto_box` is a thin libsodium shim (optional at build time), `keepass_bridge` is the socket client, and `autofill_controller` plus the injected script are the §13.2 layer. Autofill is a policy feature. Encrypted-at-rest storage of the association key is `credential_store`, which puts it in the session's Secret Service (libsecret, optional at build time) rather than inventing storage of its own — an "app-encrypted config" the app can open unattended has to keep its key on disk beside it, which is obfuscation wearing the word encryption. Without it the pairing still works and simply does not survive a restart, and the app says so. All three of what this used to list as not built are built and wired, through `autofill_controller`: `set-login` as `keepass_bridge::save_login`, `generate-password` as `keepass_bridge::generate_password`, and the entry picker as the `QInputDialog::getItem` in `main_window::wire_autofill` that asks "Which login?" when a site has more than one. What remains unexercised is the whole of it against a live KeePassXC, as the status line above says.
...
**Cookie-consent banners are a blocking option** (`cookieNotices`), not a cookie setting: the question is what a site may put in front of the page, not what it may store. Blocked by default, meaning the banner is *answered* — least permissive option it actually offers, accept only where that is the sole exit — and then whatever the click left behind is hidden, including a document-level scroll lock. Answering has to persist or the banner returns every load, and a consent choice is recorded in a cookie, so a dismissal relaxes **first-party** cookies for that host and never third-party, written as an ordinary per-site rule so the shield shows and can revert it. The rules live in a file rather than the binary, with per-rule provenance, because the intended direction is that users exchange them; that file is `site_rules`, shared with the anti-adblock detector names rather than split per feature — they are the same kind of perishable fact about how sites behave, they go stale the same way, and two files would be two provenance models to keep honest and two things to send; a rule with no host is generic, is flagged on insertion as a candidate for the shipped defaults, and the generic shape-based detector — not the vendor selector list — is what has to carry banners nobody has seen.

**One channel per page.** Every content script shares a single `QWebChannel`, built by a bootstrap injected once and handed out via `window.hydraChannel(cb)`. Constructing one per script is not merely wasteful: a QWebChannel claims the transport's `onmessage`, so the last one built answers every handshake and the rest hang. That was a live defect for as long as there were two such scripts.
