...
- **`test/*.cpp`** — offline suites. Deterministic, no display needed. These
  are the ones to run routinely.
- **`test/live/try_*.cpp`** — live drivers. They build the *whole shell* and
...
| `test_dlheaders` | `python3 test/echodl.py 8851` — same, but as the response *body*, so a completed download is the record of what arrived |
...
| `try_handoff` | nothing | handing an address to another application, proved by that application *fetching* it from a server the test runs — checking `openUrl` returned true would prove almost nothing. **Launches your default browser** on a local page |
| `try_lock` | nothing | a locked tab through the real shell: the pin takes the page that is *showing* rather than the address the node was created with, Back and Forward grey out, navigating opens a sub-tab below instead of moving the tab, and the sub-tab then browses freely. Writes five PNGs to `HYDRA_TEST_OUT` |
| `try_rename` | nothing | renaming through the real properties dialog: the typed name reaches the tree and is marked chosen, the page cannot take it back, Cancel changes nothing, and emptying the field hands the tab back to the page |
